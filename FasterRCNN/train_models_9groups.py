# train models that detect 9 groups of meiofauna
import torch
import torchvision
...
# Get current time for logging and date
current_time = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

# Define paths
dataset_path = "dataset"
model_save_path = "9groups_models_trained"
log_path = os.path.join(model_save_path, f"9groups_training_log_{current_time}.txt")
...
train_dataset = get_coco_dataset(
    ...
    ann_file=os.path.join(dataset_path, "train/annotations/meiobenthos_train_9groups_augmented_coco.json")
)
...
lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.1) #controls how much the model's weights change in response to the loss gradient, reduces the learning rate by 10x every 15 epochs (step_size=15)
...
with open(log_path, "a") as log_file:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        checkpoint_path = os.path.join(model_save_path, f"fasterrcnn_resnet50_epoch_{epoch + 1}_9groups.pth")
